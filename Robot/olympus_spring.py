...
class OlympusSpring:
    def __init__(
        ...
        self.front_motors_joint_indices.cuda(0)
        self.back_motors_joint_indices.cuda(0)
        self.indicies.cuda(0)
        self.batched_indicies.cuda(0)
    ...
    def forward(self) -> ArticulationAction:
        ...
        return ArticulationAction(
            joint_efforts=joint_efforts.clone(),
    ...
    def _get_action_front_normal_back_inverted(self, mask) -> ArticulationAction:
        ...
        cos_angle_1 = (torch.bmm(normalize(r_fk_bm).unsqueeze(1), normalize(r_fk_fm).unsqueeze(2))).squeeze().clamp(-1,1)
    ...
    def _get_mode(self) -> Tensor:
        ...
        front_knee_below = r_bk_bm[:,2] > self.r_pulley
        back_knee_below = r_fk_fm[:,2] > self.r_pulley
