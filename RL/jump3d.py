...
class Jump3DTask(RLTask):
    def __init__(self, name, sim_config, env, offset=None) -> None:
        ...
        # curiculum
        self._curriculum_init_squat_angle_lower = torch.tensor([90.0,0], device=self._device).deg2rad()
    ...
    def apply_control(self, actions) -> None:
        ...
        #dorce no movement while we wait for the agent to be still on the ground
    ...
    def calculate_metrics(self) -> None:
        ...
        rew_spin = -torch.sum(body_spin**2, dim=1)*10000
        rew_spin[~self._takeoff_buf] = 0
            
        
        
        total_reward = (rew_fallen + rew_jump + rew_squat*0 + rew_stepping + rew_symmetry + rew_spin) * self.rew_scales["total"]
